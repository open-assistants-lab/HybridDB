...
def _default_embedding_fn(text: str) -> list[float]:
    """Default embedding function using hash-based fallback if no model is available."""
    if not text:
...
def _hash_embedding(text: str) -> list[float]:
    """Deterministic hash-based embedding for offline/no-model scenarios."""
    if not text:
        return [0.0] * EMBEDDING_DIM
...
class HybridDB:
    ...
    def create_table(self, table: str, columns: dict[str, str]) -> None:
        """Create a table with typed columns and auto-setup FTS5 + ChromaDB.

        Args:
            table: Table name.
            columns: Dict of column_name -> type_spec. Supported types:
                     TEXT, LONGTEXT, INTEGER, REAL, BOOLEAN, JSON.

        TEXT columns get FTS5 keyword search.
    ...
    def add_column(self, table: str, column: str, col_type: str) -> None:
        """Add a column to an existing table. LONGTEXT columns get ChromaDB setup."""
        meta = self._table_meta(table)
        if not meta:
    ...
    def drop_column(self, table: str, column: str) -> None:
        """Drop a column from a table, rebuilding FTS5 and ChromaDB indexes."""
        meta = self._table_meta(table)
    ...
    def rename_column(self, table: str, old_name: str, new_name: str) -> None:
        """Rename a column. FTS5 indexes are rebuilt under the new name."""
        meta = self._table_meta(table)
    ...
    def insert(self, table: str, data: dict, sync: bool = True) -> int:
        ...
        If sync=True (default), ChromaDB indexes are updated immediately.
        Set sync=False for batch inserts — call process_journal() afterward.
        """
        meta = self._table_meta(table)
        if not meta:
            raise ValueError(f"Table '{table}' not found")
    ...
    def insert_batch(self, table: str, rows: list[dict], sync: bool = True) -> list[int]:
        """Insert multiple rows in a single transaction. Returns list of row IDs."""
        meta = self._table_meta(table)
        if not meta:
            raise ValueError(f"Table '{table}' not found")
    ...
    def query(
        ...
        """Query rows with optional WHERE, ORDER BY, and LIMIT clauses."""
    ...
    def search(
        ...
        Args:
            table: Table name.
            column: Column to search (must be TEXT or LONGTEXT).
            query: Search query.
            mode: SearchMode.KEYWORD, SearchMode.SEMANTIC, or SearchMode.HYBRID.
            where: Optional ChromaDB metadata filter dict.
            limit: Max results.
            fts_weight: Weight for keyword vs semantic in hybrid mode (0-1).
            recency_weight: Boost by recency (0 = off, 0.3 = moderate).
            recency_column: Column name for recency scoring (ISO timestamp).

        Returns:
            List of dicts with `_score` and `_search_mode` fields added.
        """
    ...
    def search_all(
        ...
        """Search across ALL text and longtext columns in a table."""
        pending = self._journal_count(table)
        if pending > 0:
            self._process_journal()
    ...
    def reconcile(self, table: str) -> dict:
        """Reconcile ChromaDB with SQLite: delete ghost docs, add missing docs.

        Returns a dict with 'ghosts_deleted', 'missing_added', 'metadata_updated' counts.
        """
        ...
        for col in lt_cols:
            ...
            try:
                ...
                logger.warning(
                    "reconcile_failed table=%s column=%s error=%s", table, col, e
                )

        return result

    def health(self, table: str) -> dict:
        """Check health of a table across SQLite and ChromaDB.

        Returns:
            Dict with 'sqlite_rows', 'chroma_docs', 'pending_journal', and 'status'.
            Status: 'ok', 'drift' (need reconcile), or 'broken' (chromadb unreachable).
        """
        with self._connect() as cur:
